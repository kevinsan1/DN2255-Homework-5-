...
ip(1:(N-1)) = 2:N;  ip(N) = N-1;   % ip = i+1 with periodic b.c.
im(2:N) = 1:(N-1);  im(1) = 2;   % im = i-1 with periodic b.c.
...
for i = 1:iplot
    plot(x,aplot(:,1),'-',x,aplot(:,i),'--');
    legend('Initial  ','Final');
    xlabel('x');  ylabel('a(x,t)');
    pause(.001);    % Pause 1 second between plots
end
